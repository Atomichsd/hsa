...
# لاگ‌های اضافی کتابخونه‌ها رو هم کم کن
...
API_ID = "30328260"
API_HASH = "5baebab071b3497399c8615bacd566ee"
BOT_TOKEN = "8897686953:AAEjf5jn879MjRUKZLcPY_VrNhtghSPGcFQ"
OWNER_IDS = [8447611962]
...
SOCIAL_PLATFORMS = {
    ...
    'tiktok':    ('🎵', 'TikTok'),
    'youtube':   ('▶️', 'YouTube'),
    'telegram':  ('✈️', 'Telegram (official)'),
    'other':     ('🌐', 'Other'),
}

SOCIAL_REASONS = {
    'instagram': ['Spam', 'Scam', 'Harassment', 'Hate Speech', 'Nudity', 'Impersonation', 'Copyright', 'Other'],
    'facebook':  ['Spam', 'Scam', 'Harassment', 'Hate Speech', 'Violence', 'Fake Account', 'Copyright', 'Other'],
    'twitter':   ['Spam', 'Scam', 'Harassment', 'Hate Speech', 'Violence', 'Impersonation', 'Sensitive Media', 'Other'],
    'tiktok':    ['Spam', 'Scam', 'Harassment', 'Hate Speech', 'Nudity', 'Dangerous Acts', 'Other'],
    'youtube':   ['Spam', 'Scam', 'Harassment', 'Hate Speech', 'Violence', 'Copyright', 'Other'],
    'telegram':  ['Spam', 'Fake', 'Violence', 'Pornography', 'Child Abuse', 'Copyright', 'Personal Details', 'Other'],
    'other':     ['Spam', 'Scam', 'Harassment', 'Hate Speech', 'Other'],
}

SOCIAL_REPORT_LINKS = {
    'instagram': 'https://help.instagram.com/  (Help Center → Report Something)',
    'facebook':  'https://www.facebook.com/help/contact/485974059259751',
    'twitter':   'https://help.twitter.com/en/forms/abusiveuser',
    'tiktok':    'https://support.tiktok.com/en/safety-hc/report-a-problem',
    'youtube':   'https://support.google.com/youtube/answer/2802027',
    'telegram':  'https://telegram.org/abuse',
    'other':     'Use the platform\'s built-in Report feature.',
}

SOCIAL_INAPP_STEPS = {
    'instagram': 'اینستاگرام: پروفایل/پست → ⋯ → Report → گزینهی مناسب را انتخاب کن.',
    'facebook': 'فیسبوک: پست/پروفایل → ⋯ → Find support or report → گزینهی مناسب.',
    'twitter': 'ایکس: توییت/پروفایل → ⋯ → Report → دستهبندی را انتخاب کن.',
    'tiktok': 'تیکتاک: ویدیو/پروفایل → Share → Report → گزینهی مناسب.',
    'youtube': 'یوتیوب: ویدیو → ⋯ (بیشتر) → Report → دلیل را انتخاب کن.',
    'telegram': 'تلگرام: کانال/پیام → ⋯ → Report → دلیل را انتخاب کن.',
    'other': 'در خود پلتفرم، گزینهی Report را پیدا کن.',
}

SOCIAL_REPORT_HISTORY = {}
    ...
    try:
        data = {
            ...
            'social_reports': {
                ...
                for k, v in SOCIAL_REPORT_HISTORY.items()
            }
        }
...
def load_data():
    global ADMINS, REFERRALS, REPORT_HISTORY, MANUAL_REPORT_DATES, SOCIAL_REPORT_HISTORY
    try:
        if os.path.exists('data.json'):
            ...
            SOCIAL_REPORT_HISTORY.clear()
            for k, v in data.get('social_reports', {}).items():
                SOCIAL_REPORT_HISTORY[int(k)] = [
                    {
...
def mask_number(number: str) -> str:
    # نمايش شماره با xxx به جاي 3 رقم آخر
...
async def show_accounts_page(event, user_id, page, list_type='all'):
    # نمايش 717 شماره (3 رقم آخر = xxx) - صفحه بندي شده، بدون نياز به پوشه
    NL = chr(10)
    total = len(ACCOUNT_NUMBERS)
    total_pages = max(1, -(-total // ACCOUNTS_PER_PAGE))
    page = max(0, min(page, total_pages - 1))

    start = page * ACCOUNTS_PER_PAGE
    page_nums = ACCOUNT_NUMBERS[start:start + ACCOUNTS_PER_PAGE]

    lines = []
    for n in page_nums:
        lines.append("📱 **" + mask_number(n) + "**")

    if list_type == 'all':
        header = "📋 **Your Accounts (" + str(total) + "):**" + NL + NL
    else:
        header = "📋 **Shared Accounts (" + str(total) + "):**" + NL + NL

    message = header + NL.join(lines)
    message += NL + NL + "📄 **Page " + str(page + 1) + "/" + str(total_pages) + "**"

    nav = []
    if page > 0:
        nav.append(Button.inline("◀️ Prev", 'accpage_' + list_type + '_' + str(page - 1)))
    if page < total_pages - 1:
        nav.append(Button.inline("Next ▶️", 'accpage_' + list_type + '_' + str(page + 1)))
...
async def show_social_menu(event, user_id):
    buttons = []
    row = []
    for key, (emoji, name) in SOCIAL_PLATFORMS.items():
        row.append(Button.inline(f"{emoji} {name}", f'soc_plat_{key}'))
        if len(row) == 2:
            buttons.append(row)
            row = []
    if row:
    ...
    await event.reply(
        ...
        "━━━━━━━━━━━━━━━━━━━━\n\n"
        "Choose the platform:\n\n"
        "⚠️ Only **Telegram** can be reported automatically by this bot.\n"
        "For the other platforms you will receive a professional **Report Kit** "
        "(official link + copy-paste text + in-app steps).",
        buttons=buttons
    )


async def show_social_reasons(event, state):
    platform = state.get('platform')
    reasons = SOCIAL_REASONS.get(platform, SOCIAL_REASONS['other'])
    buttons = []
    row = []
    for i, reason in enumerate(reasons):
        row.append(Button.inline(reason, f'soc_reason_{platform}_{i}'))
        if len(row) == 2:
            buttons.append(row)
            row = []
    if row:
        buttons.append(row)
    buttons.append([Button.inline("🔙 Back", b'soc_report')])
    emoji, name = SOCIAL_PLATFORMS.get(platform, ('🌐', 'Platform'))
    await event.reply(
        f"{emoji} **{name} — Report Reason**\n\n"
        "━━━━━━━━━━━━━━━━━━━━\n\n"
...
async def show_social_report_result(event, state):
    platform = state.get('platform', 'other')
    target = state.get('soc_target', '?')
    reason = state.get('soc_reason', 'Other')
    emoji, name = SOCIAL_PLATFORMS.get(platform, ('🌐', 'Other'))
    link = SOCIAL_REPORT_LINKS.get(platform, '')
    steps = SOCIAL_INAPP_STEPS.get(platform, '')

    report_text = (
        f"I am reporting the following {name} account/content for {reason}.\n"
        f"Target: {target}\n"
        f"Reason: {reason}\n"
        f"Please review this against the Community Guidelines and take the appropriate action."
    )

    message = (
        f"{emoji} **{name} — Report Kit**\n"
        f"━━━━━━━━━━━━━━━━━━━━\n\n"
        f"🎯 **Target:** {target}\n"
        f"📋 **Reason:** {reason}\n\n"
        f"📝 **Ready-to-copy text:**\n"
        f"`{report_text}`\n\n"
        f"🔗 **Official report link:**\n{link}\n\n"
        f"👣 **In-app steps:**\n{steps}\n\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"📌 Note: this bot can only automate reports on Telegram. "
        f"For {name}, submit the report manually using the kit above — "
        f"automated mass-reporting is against these platforms' Terms of Service."
    )

    try:
        SOCIAL_REPORT_HISTORY.setdefault(event.sender_id, []).append({
            'platform': platform,
            'target': target,
            'reason': reason,
            'time': datetime.now(pytz.timezone('Asia/Tehran')),
        })
        SOCIAL_REPORT_HISTORY[event.sender_id] = SOCIAL_REPORT_HISTORY[event.sender_id][-20:]
        save_data()
    except Exception as e:
        logger.error(f"Error saving social report history: {e}")

    await event.reply(message)
    await send_to_report_group(
        event.client,
        f"Social Report ({name})",
        target,
        None,
        f"Platform: {name}\nReason: {reason}"
    )
...
async def show_main_menu(event, user_id: int):
    ...
    if is_owner(user_id):
        buttons = [
            ...
            # Row 12: Full width (Social Media Report)
            [Button.inline("🌐 Report Social Media", b'soc_report')],
        ...
        buttons = [
            ...
            # Row 11: Full width (Social Media Report)
            [Button.inline("🌐 Report Social Media", b'soc_report')],
...
async def show_admin_duration_buttons(event):
    buttons = [
        ...
        [Button.inline("♾️ Lifetime", b'duration_lifetime')],
...
# ========== اصلاح شده: پشتیبانی از رفرال ==========
async def send_start_to_bot_with_unblock(client, entity, start_param=None):
    try:
        ...
        start_message = "/start"
        if start_param:
            start_message = f"/start {start_param}"
        
        try:
            await client.send_message(entity, start_message)
            return True, None
...
# ========== اصلاح شده: استخراج رفرال و ارسال ==========
async def execute_start_bot_operation(event, state):
    target = state['target']
    sessions = state['sessions']
    
    # استخراج نام ربات و پارامتر start
    bot_identifier = target
    start_param = None
    
    if '?start=' in target:
        parts = target.split('?start=')
        bot_part = parts[0]
        start_param = parts[1]
        bot_identifier = bot_part.replace('https://t.me/', '').replace('t.me/', '').replace('@', '')
        if '/' in bot_identifier:
            bot_identifier = bot_identifier.split('/')[0]
    elif 't.me/' in target or '@' in target:
        bot_identifier = target.replace('https://t.me/', '').replace('t.me/', '').replace('@', '')
        if '/' in bot_identifier:
            bot_identifier = bot_identifier.split('/')[0]
    else:
        bot_identifier = target.replace('@', '')
    
    if not bot_identifier.startswith('@'):
    ...
    await event.reply(f"🚀 **Starting bot...**\nBot: {bot_identifier}\nReferral param: {start_param if start_param else 'None'}")
    ...
    for i, session_file in enumerate(sessions, 1):
        try:
            ...
            try:
                ...
                if not target_info:
                    target_info = await get_entity_info(client, bot_identifier)
                ...
                success, error = await send_start_to_bot_with_unblock(client, entity, start_param=start_param)
    ...
    result_msg = (
        f"📊 **Bot Started**\n\n"
        f"━━━━━━━━━━━━━━━━━━━━\n\n"
        f"🤖 **Bot:** {bot_identifier}\n\n"
        f"🔗 **Referral Param:** {start_param if start_param else 'None'}\n\n"
        f"👥 **Accounts:** {len(sessions)}\n\n"
        f"✅ **Success:** {total_success}\n\n"
        f"❌ **Failed:** {total_fail}\n\n"
        f"━━━━━━━━━━━━━━━━━━━━"
...
async def main():
    ...
    async def start_handler(event):
        user_id = event.sender_id
        logger.info(f"🆔 START from user_id={user_id} | username=@{getattr(event.sender, 'username', '?')}")
        print(f"🆔 START from user_id={user_id}")
    ...
    async def message_handler(event):
        ...
        try:
            ...
            # ========== اصلاح شده: accept links with ?start= ==========
            elif step == 'startbot_target':
                if not (text.startswith('@') or text.startswith('https://t.me/') or text.startswith('t.me/')):
                    await event.reply("❌ Invalid format. Enter @username or https://t.me/username (with optional ?start=...).")
                    return
            ...
            elif step == 'soc_target':
                if not text.strip():
                    await event.reply("❌ Please enter a valid username or link:")
                    return
                state['soc_target'] = text.strip()
                state['step'] = 'soc_reason'
                await show_social_reasons(event, state)
        ...
        try:
            if data == 'back_menu':
                ...
                for admin_id, info in ADMINS.items():
                    ...
                    remaining = "♾️ Lifetime" if is_lifetime else f"{days_left}d {hours_left % 24}h left"
                    
                    admin_list.append(
                        f"👤 **ID:** `{admin_id}`\n\n"
                        f"📅 **Activated:** {activated.strftime('%Y-%m-%d %H:%M:%S')}\n\n"
                        f"⏰ **Expires:** {('♾️ Lifetime' if is_lifetime else expires.strftime('%Y-%m-%d %H:%M:%S'))}\n\n"
                        f"📊 **Status:** {status} ({remaining})\n\n"
                        f"━━━━━━━━━━━━━━━━━━━━"
                    )
                ...
                duration_map = {
                    ...
                    'duration_lifetime': timedelta(days=365 * 75)
                }
            ...
            elif data == 'soc_report':
                await show_social_menu(event, user_id)

            elif data.startswith('soc_plat_'):
                platform = data[len('soc_plat_'):]
                if platform not in SOCIAL_PLATFORMS:
                    await event.answer("❌ Invalid platform!", alert=True)
                    return
                USER_STATE[user_id] = {'step': 'soc_target', 'type': 'soc_report', 'platform': platform}
                await event.reply(
                    f"🔗 Enter the {SOCIAL_PLATFORMS[platform][1]} username or link\n"
                    f"(e.g. @username or https://...):"
                )

            elif data.startswith('soc_reason_'):
                parts = data.split('_')
                try:
                    platform = parts[2]
                    idx = int(parts[3])
                ...
                state['soc_reason'] = reasons[idx]
                await show_social_report_result(event, state)
                USER_STATE.pop(user_id, None)
                await show_main_menu(event, user_id)
                ...
                if data == 'addaccount':
                    ...
                    await show_accounts_page(event, user_id, 0, 'all')
                
                elif data == 'listsharedaccount':
                    await show_accounts_page(event, user_id, 0, 'shared')
                ...
                if data in ['report', 'report_post', 'report_profile', 'report_bot', 
                    ...
                    await event.reply(
                        ...
                        f"📅 **Activated:** {activated.strftime('%Y-%m-%d %H:%M:%S')}\n\n"
                        f"⏰ **Expires:** {('♾️ Lifetime' if is_lifetime else expires.strftime('%Y-%m-%d %H:%M:%S'))}\n\n"
                        f"📊 **Time Remaining:** {'♾️ Lifetime' if is_lifetime else f'{days_left} days ({hours_left} hours)'}\n\n"
                        f"━━━━━━━━━━━━━━━━━━━━"
                    )
            
            elif data.startswith('accpage_'):
                parts = data.split('_')
                try:
                    list_type = parts[1]
                    page = int(parts[2])
                ...
                if list_type == 'shared':
                    await show_accounts_page(event, user_id, page, 'shared')
                else:
                    await show_accounts_page(event, user_id, page, 'all')
